...
public class AlarmEmailUrlResponseBuilder extends AbstractAlarmEmailBuilder {

    private final static String SUMMARY_CONTENT = """
            ${company} ${app.portal}-${app.front}-${app.name} ${step} 步骤 ${datetime} 出现URL响应时间异常，告警ID：${alarm.id}，告警详情如下：
            """;

    private final static String SUMMARY_RESTORED = """
            ${company} ${app.portal}-${app.front}-${app.name} ${step} 步骤 ${datetime} 出现URL响应时间异常，告警ID：${alarm.id}，告警恢复时间：${restored.time}，告警详情如下：
            """;

    private final SchemaFactory schemaFactory = new SchemaFactory().string("终端IP", "操作员").datetimeField("时间").string("关键页面", "URL响应时间");

    public AlarmEmailUrlResponseBuilder() {
        super("URL响应性能告警");
    }
    ...
    public void createTable(ApeAlarm alarm, AlarmEmailRenderRecordset recordset) {
        ...
        recordset.addRecord("URL响应时间标准阈值", new DigitalFormatter(alarm.getNumberData("m_sla")).toFormatted() + " ms");
        recordset.addRecord("URL响应时间异常数量", new DigitalFormatter(alarm.getNumberData("m_cnt_slow")).toInteger() + " 次");
        recordset.addRecord("页面调用次数", new DigitalFormatter(alarm.getNumberData("m_cnt_total")).toInteger() + " 次");
        recordset.addRecord("占比", new DigitalFormatter(alarm.getNumberData("m_slow_rate")).toInteger() + " %");
    ...
    protected Recordset createDetailSet(Collection<ApeAlarmDetail> apeAlarmDetails) {
        ...
        apeAlarmDetails.forEach(alarmDetail -> new RecordFactory(recordset)
                ...
                .set("URL响应时间", new DigitalFormatter(alarmDetail.getNumberData("m_slow_time")).toFormatted() + " ms")
        );
        return recordset;
    }
