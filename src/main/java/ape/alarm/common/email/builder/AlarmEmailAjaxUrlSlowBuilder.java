...
public class AlarmEmailAjaxUrlSlowBuilder extends AbstractAlarmEmailBuilder {

    private final static String SUMMARY_CONTENT = """
            ${company} ${ajax.portal}-${ajax.front}-${ajax.name} ${step.url} 调用的 ${url.name} - ${step.ajax} Ajax ${datetime} 出现Ajax响应时间异常，告警ID：${alarm.id}，告警详情如下：
            """;
    private final static String SUMMARY_RESTORED = """
            ${company} ${ajax.portal}-${ajax.front}-${ajax.name} ${step.url} 调用的 ${url.name} - ${step.ajax} Ajax ${datetime} 出现Ajax响应时间异常，告警ID：${alarm.id}，告警恢复时间：${restored.time}，告警详情如下：
            """;

    private final SchemaFactory schemaFactory = new SchemaFactory().string("终端IP", "操作员").datetimeField("时间").string("被调用Ajax", "Ajax响应时间");

    public AlarmEmailAjaxUrlSlowBuilder() {
        super("Ajax响应性能告警");
    }
    ...
    public void createTable(ApeAlarm alarm, AlarmEmailRenderRecordset recordset) {
        ...
        recordset.addRecord("Ajax响应时间阈值标准", new DigitalFormatter(alarm.getNumberData("m_sla")).toFormatted() + " ms");
        ...
        recordset.addRecord("占比", new DigitalFormatter(alarm.getNumberData("m_slow_rate")).toFormatted() + " %");
    ...
    protected Recordset createDetailSet(Collection<ApeAlarmDetail> apeAlarmDetails) {
        ...
        apeAlarmDetails.forEach(alarmDetail -> new RecordFactory(recordset)
                ...
                .set("被调用Ajax", alarmDetail.getAjaxUrl())
                .set("Ajax响应时间", new DigitalFormatter(alarmDetail.getNumberData("m_slow_time")).toFormatted() + " ms")
        );
        return recordset;
    }
