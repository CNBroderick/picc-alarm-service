...
public class ApeAlarmUpdateSendStatus extends JdbcUpdateOperation implements HasEntitiesParameter<ApeAlarm> {

    //language=MySQL
    private final static String UPDATE_SEND_STATUS_SQL = """
            UPDATE tb_alarm SET d_send_status = ?, d_send_time = NOW() WHERE d_id = ?;
            """;
    ...
    public IDBAccessCallback createCallBack() {
        return db -> updateSendStatus(
                ...
                getEntities(getContext(), "apeAlarm"),
                getContext().getObject("apeAlarmSendStatus")
        );
    }

    public void updateSendStatus(Connection connection, Collection<ApeAlarm> alarms, ApeAlarmSendStatus sendStatus) throws Exception {
        new PreparedStatementHelper(connection, UPDATE_SEND_STATUS_SQL)
                .addBatch(alarms, alarm -> new Object[]{sendStatus.name(), alarm.getId()})
                .executeBatch()
        ;
    }
}
